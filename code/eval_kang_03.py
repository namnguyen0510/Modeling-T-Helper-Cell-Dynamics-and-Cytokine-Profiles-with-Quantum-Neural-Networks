...
data = pd.read_csv('clinical_datasets/1. kang_HIV-HCV/kang_05.csv')

def get_ytrue(data, cytokine):
    data = data[data['Cell Type'] == cytokine]
    # Q3/Max
    y_true = data.iloc[:,1].to_numpy().astype('float')
    y_true = MinMaxNormalization(y_true)
    return y_true

y_true_th1 = get_ytrue(data, 'Th1')
y_true_th2 = get_ytrue(data, 'Th2')
print(y_true_th1)
print(y_true_th2)
y_true = np.array([y_true_th1,y_true_th2])
...
for epoch in range(epochs):
    ...
    I4 = dynamics[5]
    # PREDICTION
    y_pred = np.array([MinMaxNormalization(Th1), MinMaxNormalization(Th2)]).astype('float')
    #y_pred = MinMaxNormalization(y_pred)
    y_pred = y_pred[:,[0,28,84,168,365]]
...
tw = [0, 4, 12, 24, 48]
t = np.linspace(0, shoots, len(Th1))/7

fig, axs = plt.subplots(2, 3, figsize=(12, 8))

axs[0,0].plot(t, Th1, label='Th1', color = plt.cm.Set1(0))
axs[0,0].plot(t, Th2, label='Th2', color = plt.cm.Set1(1))
axs[0,0].scatter(tw, y_true[0], marker = '*', alpha = 0.5, s= 100, color = plt.cm.Set1(0))
axs[0,0].scatter(tw, y_true[1], marker = '*', alpha = 0.5, s= 100, color = plt.cm.Set1(1))

axs[0,0].set_xlabel('Time [week]', fontweight = 'bold', fontsize = 14)
axs[0,0].set_ylabel('Normalized Level [pg/ml]', fontweight = 'bold', fontsize = 14)
