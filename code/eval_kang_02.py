...
exp_name = '2.2.2_Q4' 
#group_name = 'HIV/HCV co-infection'
group_name = 'HIV mono-infection'


data = pd.read_csv('clinical_datasets/1. kang_HIV-HCV/kang_03.csv')
data = data[data['Group'] == group_name]

def get_ytrue(data, cytokine):
    data = data[data['Cytokine'] == cytokine]
    # Q3/Max
    y_true = data.iloc[:,4].to_numpy().astype('float')
    y_true = MinMaxNormalization(y_true)
    return y_true

y_true_inf = get_ytrue(data, 'INF-gamma')
y_true_i2 = get_ytrue(data, 'IL-2')
y_true_i10 = get_ytrue(data, 'IL-10')
y_true_i4 = get_ytrue(data, 'IL-4')
print(y_true_inf)
print(y_true_i2)
print(y_true_i10)
print(y_true_i4)
y_true = np.array([y_true_inf,y_true_i2,y_true_i10,y_true_i4])



ratio_df = pd.read_csv('clinical_datasets/1. kang_HIV-HCV/kang_04.csv')
ratio = ratio_df[ratio_df['Group'] == group_name]
print(ratio)
mean_ratio = ratio.iloc[:,2].mean()
...
for epoch in range(epochs):
    ...
    I4 = dynamics[5]
    # PREDICTION
    y_pred = np.array([Igamma, I2, I10, I4]).astype('float')
    y_pred = MinMaxNormalization(y_pred)
    #y_pred = y_pred[:,[0,27,83,168,364]]
    y_pred = y_pred[:,[0,3,11,23,47]]
    print(y_pred)
...
axs[0,0].set_xlabel('Time [week]', fontweight = 'bold', fontsize = 14)
...
axs[1,0].scatter(tw, y_true[0], marker = '*', alpha = 0.5, s= 100, color = plt.cm.Set2(0))
axs[1,0].scatter(tw, y_true[1], marker = '*', alpha = 0.5, s= 100, color = plt.cm.Set2(1))
axs[1,0].scatter(tw, y_true[2], marker = '*', alpha = 0.5, s= 100, color = plt.cm.Set2(2))
axs[1,0].scatter(tw, y_true[3], marker = '*', alpha = 0.5, s= 100, color = plt.cm.Set2(3))
axs[1,0].set_xlabel('Time [week]', fontweight = 'bold', fontsize = 14)
...
axs[1,0].grid(True)
axs[1,0].set_xticks(tw)

axs[0,1].plot(t, Th1/Th2, label='Th1/Th2 Ratio', color = plt.cm.Set1(0))
axs[0,1].axhline(y = mean_ratio, ls = '--', label = 'Mean Ratio', color = plt.cm.Set1(1))
'''
axs[0,1].axhline(y = mean_ratio, ls = '--', label = 'Q1', color = plt.cm.Set2(2))
...
'''
axs[0,1].set_xlabel('Time [week]', fontweight = 'bold', fontsize = 14)
...
axs[0,1].grid(True)
axs[0,1].set_xticks(tw)


axs[0,2].scatter(I2, Igamma, label= r'[1] IL-2 v.s [2] IFN-$\gamma$', color = plt.cm.Set1(0), s = 20)
axs[0,2].scatter(I4, I10, label='[1] IL-4 v.s [2] IL-10',color =  plt.cm.Set1(1), s = 20)
...
axs[1,1].scatter(Igamma, I4, label= r'[1] IFN-$\gamma$ v.s [2] IL-4',  color = plt.cm.Set2(0), s = 20)
axs[1,1].scatter(Igamma, I10, label= r'[1] IFN-$\gamma$ v.s [2] IL-10',  color = plt.cm.Set2(1), s = 20)
