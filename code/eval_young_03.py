...
# Set random seeds for reproducibility 44
seed = 42
np.random.seed(seed)
torch.manual_seed(seed)

exp_name = '3.3.9_' 
group_name = 'Insuline Resistance'

_classes = ['Negative', 'Positive']

for classes in _classes:

    data = pd.read_csv('clinical_datasets/2. young/young_02.csv')
    data = data[data['Disease'] == group_name]
    data = data[data['Class'] == classes]
    y_true = data.iloc[:,[2,4]].to_numpy().astype('float')[0]
    print(data)
    print(y_true)
    ...
    for epoch in range(epochs):
        ...
        I4 = dynamics[5]
        # PREDICTION
        y_pred = np.array([Th1[-1], Th2[-1]]).astype('float')
        ...
        if loss < best_loss:
            ...
            out_df['I10'] = best_I10
            out_df['I4'] = best_I4
            out_df.to_csv('{}_best_result_{}_{}.csv'.format(exp_name, group_name,classes), index = False)
            # Save the model
            torch.save(model.state_dict(), '{}_best_model_{}_{}.pth'.format(exp_name, group_name,classes).format(exp_name))
    print(best_loss)
    ...
    axs[0,0].plot(t[-1], y_true[0], marker = '*', markersize = 20, color = plt.cm.Set1(0))
    axs[0,0].plot(t[-1], y_true[1], marker = '*', markersize = 20, color = plt.cm.Set1(1))
    ...
    plt.tight_layout()
    plt.savefig('{}_best_result_{}_{}.jpg'.format(exp_name, group_name,classes), dpi = 600)
    #plt.show()
